...
export const LARK_COL_PERIOD = 'Reporting Period'

export const LARK_COL_OFFICE = {
  Ky: LARK_COL_PERIOD,
  CongTy: 'Company',
  ThietBi: 'Equipment',
  NguonPhatThai: 'Emission Source',
  Scope: 'Scope',
  DonVi: 'Unit',
  KhoiLuong: 'Volume',
  Ef: 'EF (kg CO₂e/unit)',
  EfRef: 'EF Reference',
  TongGhg: 'Total GHG (tonnes CO₂e)',
}
...
export const LARK_COL_TRIP = {
  ...
  XuatPhat: 'From',
  Den: 'To',
  NgayDi: 'Departure Date',
  NgayVe: 'Return Date',
  Co2Bay: 'CO₂ flight (kg)',
  Co2MatDat: 'CO₂ ground (kg)',
  Co2LuuTru: 'CO₂ accommodation (kg)',
  Tong: 'Total (kg CO₂e)',
  PhuongTienChiTiet: 'Transport details',
  LuuTruChiTiet: 'Accommodation details',
}
...
export const LARK_COL_COMMUTE = {
  ...
  PhongBan: 'Department',
  PhuongTien: 'Vehicle',
  KmMotChieu: 'One-way km',
  NgayDiLamThang: 'Working days / Month',
  Wfh: 'WFH (days/month)',
...
export const LARK_COL_CLOSE = {
  ...
  Loai: 'Type',
  CongTy: 'Company',
  Scope1: 'Scope 1 (tonnes)',
  Scope2: 'Scope 2 (tonnes)',
  Scope3: 'Scope 3 (tonnes)',
  Tong: 'Total (tonnes)',
  NgayDong: 'Period close date',
  SoVanPhong: 'Office rows',
  SoChuyen: 'Business trip rows',
  SoDiLam: 'Commute rows',
}
...
function wrapLarkNetworkError(err, url, path = '') {
  ...
  let message = `Cannot connect to Lark proxy — Failed to fetch`
  if (/content.decoding|ERR_CONTENT_DECODING/i.test(original)) {
    message =
      'Response decompression error (Content-Encoding). Restart npm run dev — proxy has been fixed; for Vercel: redeploy.'
  } else if (prefix.startsWith('http')) {
    message =
      'Cannot call Lark (CORS). On Vercel remove VITE_LARK_API_BASE pointing directly to open.larksuite.com — use /api/lark.'
  } else if (import.meta.env.DEV) {
    message = `Cannot connect to Lark proxy. Keep "npm run dev" running; F5. Tunnel: npm run tunnel + dev.`
  } else if (isLocalBrowserHost()) {
    message = 'Cannot connect to proxy. Run npm run dev — do not open HTML file directly.'
  } else {
    message = 'Cannot connect to /api/lark. Redeploy Vercel; remove VITE_LARK_API_BASE=/lark-open-api on Vercel.'
  }
...
async function larkFetch(path, init = {}, prefixOverride) {
  ...
  if (!res.ok) {
    ...
    if (looksLikeSpaHtml && prefix.startsWith('/')) {
      throwLarkError(
        'HTTP 404 — /api/lark returned HTML page (SPA). Redeploy Vercel + vercel.json; remove VITE_LARK_API_BASE=/lark-open-api.',
        {
          step,
          url,
          method,
    ...
    if (res.status === 404 && prefix.startsWith('/')) {
      throwLarkError(`HTTP 404 — Lark route not found: ${url}`, {
        step,
        url,
        method,
        httpStatus: 404,
...
export async function fetchTenantAccessToken(prefix, appId, appSecret) {
  ...
  if (!token) {
    throwLarkError('No tenant_access_token received — check App ID / Secret on Lark Open Platform', {
      step,
      url,
      method,
      apiPrefix,
...
const PERIOD_FIELD_ALIASES = [LARK_COL_PERIOD, 'Period', 'Kỳ', 'Kỳ (mã)']
...
async function batchCreateRecords(prefix, token, baseAppToken, tableId, fieldsList) {
  ...
  if (omitted.length) {
    console.info(`[Lark] Table ${tableId} — skipping columns not on Base:`, omitted.join(', '))
  }
  if (alignedList.length === 0) {
    throwLarkError('No matching columns found on Base to write — check column names on Lark.', {
      step: 'Write table',
      tableId,
  ...
  for (const part of chunk(alignedList, 80)) {
    ...
    if (json.code !== 0) {
      ...
      const title = missingOnBase.length
        ? `FieldNameNotFound — missing/wrong column name: ${missingOnBase.join(', ')}`
  ...
  if (tClose) {
    ...
    for (const co of companies) {
      ...
      summaryFields.push(
        fieldsAllText({
          [CL.Ky]: snap.label,
          [CL.Loai]: snap.type === 'year' ? 'Year' : 'Month',
          [CL.CongTy]: co || 'All',
...
export const LARK_FIELD_HELP = {
  office: [
    'ID — do not modify / not sent by app',
    `${LARK_COL_PERIOD} (optional — only sent if Base has column «Reporting Period» or «Period»)`,
