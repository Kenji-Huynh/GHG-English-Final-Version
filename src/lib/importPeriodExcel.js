...
function headerIndexMap(row, expected, optionalHeaders = EXCEL_OPTIONAL_COLUMNS) {
  ...
  for (const h of expected) {
    ...
    if (map[n] === undefined) {
      throw new Error(`Missing required column: «${h}». Check the header row of the sheet.`)
    }
  }
  return map
}
...
function rowIsPlaceholder(sheet, rowNum) {
  ...
  return t.startsWith('(') || u === 'TOTAL' || u === 'TONG' || u === 'TỔNG'
}
...
function resolveCompany(raw, defaultCompany, warnings, ctx) {
  ...
  if (n) return n
  warnings.push(`${ctx}: company «${t}» is invalid — using default or leaving blank (${COMPANIES.join(', ')})`)
...
export async function parsePeriodExcel(arrayBuffer) {
  ...
  if (wsO) {
    ...
    for (let r = 2; r <= lastRow; r++) {
      ...
      const equipment = cellToString(cellAt(row, col, 'Equipment'))
      const source = cellToString(cellAt(row, col, 'Emission Source'))
      const volume = cellToNumber(cellAt(row, col, 'Volume'))
      const ef = cellToNumber(cellAt(row, col, 'EF (kg CO₂e/unit)'))
      ...
      equip.push({
        ...
        source,
        scope: scope === 2 ? 2 : 1,
        unit: cellToString(cellAt(row, col, 'Unit')),
        volume,
        ef,
  ...
  if (wsT) {
    ...
    for (let r = 2; r <= lastRow; r++) {
      ...
      const name = cellToString(cellAt(row, col, 'Full Name'))
      const empId = cellToString(cellAt(row, col, 'Emp ID'))
      const tripName = cellToString(cellAt(row, col, 'Trip Name'))
      ...
      const co2Air = cellToNumber(cellAt(row, col, 'CO₂ flight (kg)'))
      const co2Ground = cellToNumber(cellAt(row, col, 'CO₂ ground (kg)'))
      const co2Hotel = cellToNumber(cellAt(row, col, 'CO₂ accommodation (kg)'))
      let co2Total = cellToNumber(cellAt(row, col, 'Total (kg CO₂e)'))
      ...
      trips.push({
        ...
        empId,
        company,
        dept: cellToString(cellAt(row, col, 'Department')),
        trip: tripName,
        purpose: cellToString(cellAt(row, col, 'Purpose')),
        from: cellToString(cellAt(row, col, 'From')),
        to: cellToString(cellAt(row, col, 'To')),
        dateFrom: cellToString(cellAt(row, col, 'Departure Date')),
        dateTo: cellToString(cellAt(row, col, 'Return Date')),
        proj: '',
        note: '',
  ...
  if (wsC) {
    ...
    for (let r = 2; r <= lastRow; r++) {
      ...
      const vehicle = cellToString(cellAt(row, col, 'Vehicle'))
      const km = cellToNumber(cellAt(row, col, 'One-way km'))
      const days = cellToNumber(cellAt(row, col, 'Working days / month'))
      const wfh = cellToNumber(cellAt(row, col, 'WFH (days/month)'))
      ...
      const carpool = cellToNumber(cellAt(row, col, 'Carpool (people)')) || 1
  ...
  if (!hasOfficeSheet && !hasTripSheet && !hasCommuteSheet) {
    throw new Error(
      `No data sheet found. At least one of the following is required: «${EXCEL_SHEET.office}», «${EXCEL_SHEET.trip}», «${EXCEL_SHEET.commute}».`,
    )
  }
...
export function buildImportPreviewHtml(parsed) {
  const lines = []
  if (parsed.hasOfficeSheet) lines.push(`<strong>Office:</strong> ${parsed.counts.equip} rows`)
  if (parsed.hasTripSheet) lines.push(`<strong>Business Trips:</strong> ${parsed.counts.trips} rows`)
  if (parsed.hasCommuteSheet) lines.push(`<strong>Commute:</strong> ${parsed.counts.commutes} rows`)
  if (parsed.overview.company) {
    lines.push(`<strong>Period default company:</strong> ${parsed.overview.company}`)
  }
  if (!lines.length) lines.push('No valid data rows found in file.')
  let html = `<p style="margin:0 0 8px">${lines.join('<br>')}</p>`
  html +=
    '<p style="margin:0;font-size:12px;color:#6B6960"><strong>Merge:</strong> keep existing data, only add new rows (skip duplicates).<br><strong>Replace:</strong> clear sheet data present in file then write from Excel (unconfirmed draft equipment is kept).</p>'
  if (parsed.warnings.length) {
    const show = parsed.warnings.slice(0, 5)
    html += `<p style="margin:8px 0 0;font-size:11px;color:#B85C00">⚠ ${show.join('<br>⚠ ')}${parsed.warnings.length > 5 ? `<br>… +${parsed.warnings.length - 5} warnings` : ''}</p>`
  }
  return html
}
