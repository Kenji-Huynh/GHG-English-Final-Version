...
function addOverviewSheet(workbook, pk, m, y, lastRow = EXCEL_FORMULA_LAST_ROW) {
  ...
  title.value = 'GHG Inventory — Period Emissions Report'
  ...
  const pairs = [
    ...
    ['Valid companies (select in sheet)', EXCEL_COMPANY_LIST.join(' · ')],
    ['Total Scope 1 + 2 (tonnes CO₂e)', null],
    ['Total Scope 3 — estimated (tonnes CO₂e)', null],
    ['Total all scopes (tonnes CO₂e)', null],
  ]
  ...
  for (const [label, val] of pairs) {
    ...
    if (label === 'Total Scope 1 + 2 (tonnes CO₂e)') {
      rowS12 = r
      b.value = { formula: `SUM(${off}!I2:I${lastRow})` }
      b.numFmt = '#,##0.000'
      b.font = { size: 12, bold: true, color: { argb: ACCENT } }
    } else if (label === 'Total Scope 3 — estimated (tonnes CO₂e)') {
      rowS3 = r
      b.value = {
      ...
      b.numFmt = '#,##0.000'
      b.font = { size: 12, bold: true, color: { argb: ACCENT } }
    } else if (label === 'Total all scopes (tonnes CO₂e)') {
      rowTotal = r
...
function addOverviewSheetTemplate(workbook, lastRow = EXCEL_FORMULA_LAST_ROW) {
  ...
  ws.mergeCells('A1:D1')
  const title = ws.getCell('A1')
  title.value = 'GHG Inventory — Period Emissions Report (template)'
  title.font = { size: 16, bold: true, color: { argb: 'FF1A1916' } }
  title.alignment = { vertical: 'middle', horizontal: 'left' }
  ws.getRow(1).height = 30
  ...
  const sub = ws.getCell('A2')
  sub.value =
    'Select company in the «Company» column (dropdown) on each sheet; period default is in the table below.'
  sub.font = { size: 12, bold: true, color: { argb: ACCENT } }
  sub.alignment = { vertical: 'middle', horizontal: 'left' }
  ws.getRow(2).height = 22

  const pairs = [
    ['Reporting period (code)', '(e.g. 2026-05)'],
    ['Exported at', '—'],
    [EXCEL_OVERVIEW_COMPANY_LABEL, ''],
    [EXCEL_OVERVIEW_LOCATION_LABEL, ''],
    ['Valid companies (select in sheet)', EXCEL_COMPANY_LIST.join(' · ')],
...
function addCompanyColumnValidation(ws, colLetter, lastRow = 500) {
  ws.dataValidations.add(`${colLetter}2:${colLetter}${lastRow}`, {
    ...
    error: `Select one of: ${EXCEL_COMPANY_LIST.join(', ')}`,
  })
}
...
function addOfficeSheet(workbook, rows, opts = {}) {
  ...
  if (forTemplate) {
    ...
    const row = ws.addRow(['(No confirmed rows in summary)', '', '', '', '', '', '', '', ''])
...
function addTripSheet(workbook, trips, opts = {}) {
  ...
  if (forTemplate) {
    ...
    const row = ws.addRow(['(No business trips in this period)', '', '', '', '', '', '', '', '', '', '', '', '', ''])
    row.getCell(1).font = { italic: true, color: { argb: 'FF9A9890' } }
    ws.mergeCells('A2:N2')
  } else {
    let rowNum = 1
...
function addCommuteSheet(workbook, list, opts = {}) {
  ...
  if (forTemplate) {
    ...
    const row = ws.addRow(['(No commute records in this period)', '', '', '', '', '', '', '', '', '', ''])
    row.getCell(1).font = { italic: true, color: { argb: 'FF9A9890' } }
    ws.mergeCells('A2:K2')
  } else {
    let rowNum = 1
